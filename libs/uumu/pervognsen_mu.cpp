...
// @url:
// https://chromium.googlesource.com/chromium/src/+/58e56144d4831f31dba534d538bc7ae75fe03/content/browser/gamepad/data_fetcher_win.cc#98
// @url:
// https://chromium.googlesource.com/chromium/src/+/58e56144d4831f31dba534d538bc7ae75fe03/content/browser/system_message_window_win.cc#49
...
static LRESULT CALLBACK Mu_Window_Proc(HWND window,
                                       UINT message,
                                       WPARAM wparam,
                                       LPARAM lparam)
{
  ...
  case WM_DPICHANGED:
  {
    ...
    SetWindowPos(window, 0, rect->left, rect->top, rect->right - rect->left,
  ...
  case WM_INPUT:
  {
        ...
        == size)
    {
          ...
          && raw_input->data.mouse.usFlags == MOUSE_MOVE_RELATIVE)
      {
        ...
        Mu_UpdateDigitalButtonAndPreserveTransitions(
          &mu->mouse.left_button, left_button_down);
        ...
        Mu_UpdateDigitalButtonAndPreserveTransitions(
          &mu->mouse.right_button, right_button_down);

        if (button_flags & RI_MOUSE_WHEEL)
        {
          mu->mouse.delta_wheel +=
            ((SHORT)raw_input->data.mouse.usButtonData) / WHEEL_DELTA;
        }
      }
    }
    result = DefWindowProcA(window, message, wparam, lparam);
  ...
  case WM_CHAR:
  {
    ...
    uint32_t ascii_length =
      WideCharToMultiByte(CP_ACP, 0, &utf16_character, 1, &ascii_character, 1, 0, 0);
...
Mu_Bool Mu_Window_Initialize(Mu* mu)
{
  ...
  mu->win32->message_fiber =
    CreateFiber(0, (PFIBER_START_ROUTINE)Mu_Window_MessageFiberProc, mu);
  ...
  mu->win32->window = CreateWindowA("mu", mu->window.title, WS_OVERLAPPEDWINDOW, window_x,
                                    window_y, window_width, window_height, 0, 0, 0, mu);
  if (!mu->win32->window)
  {
...
void Mu_Time_Pull(Mu* mu)
{
  ...
  mu->time.delta_nanoseconds =
    (1000 * 1000 * 1000 * mu->time.delta_ticks) / mu->time.ticks_per_second;
  ...
  mu->time.nanoseconds =
    (1000 * 1000 * 1000 * mu->time.ticks) / mu->time.ticks_per_second;
...
Mu_Bool Mu_Gamepad_Initialize(Mu* mu)
{
  ...
  if (xinput_module)
  {
    mu->win32->xinput_get_state =
      (XINPUTGETSTATE)GetProcAddress(xinput_module, "XInputGetState");
  }
...
void Mu_Gamepad_Pull(Mu* mu)
{
  ...
  if (!mu->win32->xinput_get_state
      || mu->win32->xinput_get_state(0, &xinput_state) != ERROR_SUCCESS)
  {
  ...
  Mu_UpdateDigitalButton(
    &mu->gamepad.a_button, (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_A) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.b_button, (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_B) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.x_button, (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_X) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.y_button, (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_Y) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.left_shoulder_button,
    (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.right_shoulder_button,
    (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER) != 0);
  Mu_UpdateDigitalButton(&mu->gamepad.up_button,
                         (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_UP) != 0);
  Mu_UpdateDigitalButton(&mu->gamepad.down_button,
                         (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_DOWN) != 0);
  Mu_UpdateDigitalButton(&mu->gamepad.left_button,
                         (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.right_button,
    (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.left_thumb_button,
    (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_THUMB) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.right_thumb_button,
    (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_THUMB) != 0);
  Mu_UpdateDigitalButton(
    &mu->gamepad.back_button, (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_BACK) != 0);
  Mu_UpdateDigitalButton(&mu->gamepad.start_button,
                         (xinput_state.Gamepad.wButtons & XINPUT_GAMEPAD_START) != 0);
  Mu_UpdateAnalogButton(
    &mu->gamepad.left_trigger, xinput_state.Gamepad.bLeftTrigger / 255.f);
  Mu_UpdateAnalogButton(
    &mu->gamepad.right_trigger, xinput_state.Gamepad.bRightTrigger / 255.f);
#define CONVERT(x) (2.0f * (((x + 32768) / 65535.f) - 0.5f))
  Mu_UpdateStick(&mu->gamepad.left_thumb_stick, CONVERT(xinput_state.Gamepad.sThumbLX),
                 CONVERT(xinput_state.Gamepad.sThumbLY));
  Mu_UpdateStick(&mu->gamepad.right_thumb_stick, CONVERT(xinput_state.Gamepad.sThumbRX),
                 CONVERT(xinput_state.Gamepad.sThumbRY));
#undef CONVERT
}
...
DWORD WINAPI Mu_Audio_ThreadProc(LPVOID parameter)
{
  ...
  for (;;)
  {
    ...
    if (mu->win32->audio_render_client->GetBuffer(
          fill_frame_count, (BYTE**)&buffer.samples)
        < 0)
    {
      goto done;
    }
...
static WAVEFORMATEX win32_audio_format = {
  WAVE_FORMAT_PCM,     2, 44100, 44100 * sizeof(int16_t) * 2, sizeof(int16_t) * 2,
  sizeof(int16_t) * 8, 0};
...
Mu_Bool Mu_Audio_Initialize(Mu* mu)
{
  ...
  if (CoCreateInstance(__uuidof(MMDeviceEnumerator), 0, CLSCTX_INPROC_SERVER,
  ...
  if (audio_device->Activate(
        __uuidof(IAudioClient), CLSCTX_INPROC_SERVER, 0, (void**)&audio_client)
      < 0)
  {
    goto done;
  }
  REFERENCE_TIME audio_buffer_duration = 40 * 1000 * 10; // 40 milliseconds
  DWORD audio_client_flags = AUDCLNT_STREAMFLAGS_EVENTCALLBACK
                             | AUDCLNT_STREAMFLAGS_RATEADJUST
                             | AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM;
  if (audio_client->Initialize(AUDCLNT_SHAREMODE_SHARED, audio_client_flags,
                               audio_buffer_duration, 0, &win32_audio_format, 0)
      < 0)
  {
    goto done;
  }
...
Mu_Bool Mu_D3D11_Initialize(Mu* mu)
{
  ...
  typedef HRESULT D3D11CreateDeviceAndSwapChainProc(
    _In_opt_ IDXGIAdapter * pAdapter, D3D_DRIVER_TYPE DriverType, HMODULE Software,
    UINT Flags, _In_opt_ const D3D_FEATURE_LEVEL* pFeatureLevels, UINT FeatureLevels,
  ...
  D3D11CreateDeviceAndSwapChainProc* CreateDeviceAndSwapChain =
    (D3D11CreateDeviceAndSwapChainProc*)GetProcAddress(
      d3d11_module, "D3D11CreateDeviceAndSwapChain");
  if (!CreateDeviceAndSwapChain)
  {
    mu->error = "can't find D3D11CreateDeviceAndSwapChain in d3d11.dll"; // TODO(nicolas):
                                                                         // path of dll
                                                                         // here
    return MU_FALSE;
  }
  ...
  HRESULT hr = CreateDeviceAndSwapChain(
    NULL /* default adapter */, D3D_DRIVER_TYPE_HARDWARE,
    NULL /* software renderer module */,
    0 * D3D11_CREATE_DEVICE_SINGLETHREADED | 1 * D3D11_CREATE_DEVICE_DEBUG
      | 1 * D3D11_CREATE_DEVICE_BGRA_SUPPORT,
    NULL /* feature_levels */, 0, D3D11_SDK_VERSION, &swap_chain_desc,
    &mu->win32->dxgi_swap_chain, &mu->win32->d3d11_device,
    feature_levels /* feature level */, &mu->win32->d3d11_device_context);
  if (hr != 0)
  {
...
Mu_Bool Mu_OpenGL_Initialize(Mu* mu)
{
  ...
  int pixel_format =
    ChoosePixelFormat(mu->win32->device_context, &pixel_format_descriptor);
  if (!pixel_format)
  {
    return MU_FALSE;
  }
  if (!DescribePixelFormat(mu->win32->device_context, pixel_format,
...
void Mu_Push(Mu* mu)
{
  ...
  if ((mu->window.size.x != 0 && mu->window.size.x != size.x)
      || (mu->window.size.y != 0 && mu->window.size.y != size.y))
  {
    /*
...
Mu_Bool Mu_LoadImage(const char* filename, Mu_Image* image)
{
  ...
  if (!wic_factory)
  {
    ...
    if (!wic_factory)
    {
      CoInitializeEx(0, COINITBASE_MULTITHREADED);
      if (CoCreateInstance(
            CLSID_WICImagingFactory, 0, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&wic_factory))
          < 0)
      {
  ...
  if (wic_factory->CreateDecoderFromFilename(
        wide_filename, 0, GENERIC_READ, WICDecodeMetadataCacheOnDemand, &image_decoder)
      < 0)
  {
    goto done;
  }
  ...
  if (WICConvertBitmapSource(GUID_WICPixelFormat32bppRGBA, image_frame, &rgba_image_frame)
      < 0)
  {
    goto done;
  }
...
Mu_Bool Mu_LoadAudio(const char* filename, Mu_AudioBuffer* audio)
{
  ...
  media_type->SetUINT32(
    MF_MT_AUDIO_SAMPLES_PER_SECOND, win32_audio_format.nSamplesPerSec);
  media_type->SetUINT32(MF_MT_AUDIO_BLOCK_ALIGNMENT, win32_audio_format.nBlockAlign);
  media_type->SetUINT32(
    MF_MT_AUDIO_AVG_BYTES_PER_SECOND, win32_audio_format.nAvgBytesPerSec);
  ...
  if (hr = source_reader->SetCurrentMediaType(
        MF_SOURCE_READER_FIRST_AUDIO_STREAM, 0, media_type),
      hr != 0)
  {
    goto done;
  }
  size_t buffer_capacity = mu_audio_format.samples_per_second
                           * mu_audio_format.bytes_per_sample; // 1 second of audio
  ...
  for (;;)
  {
    ...
    LONGLONG timestamp;
    IMFSample* sample;
    if (hr = source_reader->ReadSample(MF_SOURCE_READER_FIRST_AUDIO_STREAM, 0,
